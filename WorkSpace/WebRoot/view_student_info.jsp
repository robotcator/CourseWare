...
<html>
  ...
  <body>
	...
	<div id="content" style="background-color:#EEEEEE;height:320pt;width:90%;x;float:left;">
		<div align="center">
			<form name="delete_form" method="post" action="delete_student.jsp">
	    ...
	    %>
			<tr>
				...
				<input type="checkbox" name="Delete_student" value="<%=sData[0]%>">
				</td>
		...
		<input type="submit" name="sub" value="删除选中学生">
		</form>
			<form action="view_student_info.jsp" method="post" target="_self">	
			第<font color="#FF0000"><%=showPage%></font>页/共<font color=red><%=Page.getPageCount()%></font>页&nbsp;
			<a href="view_student_info.jsp?showPage=1" target="_self"><font color="#0000FF">[首页]</font></a>&nbsp;			
			<%
				//判断"上一页"链接是否要显示
				if(showPage > 1){				
			%>
					<a href="view_student_info.jsp?showPage=<%=showPage-1%>" target="_self"><font color="#0000FF">[上一页]</font> </a>&nbsp;
			<%
				}else{
					out.println("[上一页]&nbsp;");
				}
				...
				if(showPage < Page.getPageCount()){				
			%>
					<a href="view_student_info.jsp?showPage=<%=showPage+1%>" target="_self"><font color="#0000FF">[下一页]</font> </a>&nbsp;
			<%
				}else{
					out.println("[下一页]&nbsp;");
				}
			%> 
			<a href="view_student_info.jsp?showPage=<%=Page.getPageCount()%>" target="_self"><font color="#0000FF">[尾页]</font> </a>&nbsp;
			转到
